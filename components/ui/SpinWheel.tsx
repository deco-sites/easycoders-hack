...
export default function SpinWheel({ title, subtitle, spinValues }: Props) {
  function handleSpin() {
    if (IS_BROWSER) {
      ...
      const spinWheelNumber = document.querySelector(`.spin-wheel-number[data-id="${currentItem}"] span`);
      ...
      setTimeout(() => {
        if (stoppedNumber) {
          const textReturnned = couponMap[stoppedNumber] || "Opa, não foi dessa vez. Tente novamente mais tarde";
  ...
  return (
    <>
    <label for="my_modal_spin" class="fixed bottom-24 right-7 z-40 animate-pulse">
      <div class="relative px-5 py-2 font-medium text-white group mt-4 cursor-pointer">
        ...
        <span class="absolute bottom-0 left-0 hidden w-10 h-20 transition-all duration-100 ease-out transform -translate-x-8 translate-y-10 bg-orange-600 -rotate-12"></span>
        <span class="absolute bottom-0 right-0 hidden w-10 h-20 transition-all duration-100 ease-out transform translate-x-10 translate-y-8 bg-orange-400 -rotate-12"></span>
      ...
      <div class="modal-box overflow-hidden">
        <div class="flex flex-col gap-0.5 items-center justify-center">
          ...
          <div class="w-[400px] h-[400px] relative flex items-center justify-center">
            <div onClick={handleSpin} class="absolute w-[60px] h-[60px] bg-white rounded-[50%] z-10 flex items-center justify-center font-semibold text-[#333] leading-[0.1em] border-4 border-solid border-[rgba(0,0,0,0.75)] cursor-pointer select-none before:absolute before:-top-[28px] before:w-5 before:h-[30px] before:bg-white before:polygon-clip">Girar</div>
            
            <div id="wheel" class="absolute top-0 left-0 w-full h-full bg-[#333] rounded-[50%] overflow-hidden spin-wheel-shadow transform duration-1000 ease-in-out">
              {spinValues && spinValues.slice(0, 8).map((item, index) => (
                <div data-id={index + 1} style={`--i:${index+1};--clr:${item.color}`} class="absolute w-1/2 h-1/2 spin-wheel-number flex items-center justify-center select-none cursor-pointer"><span class="relative rotate-45 text-lg text-white shadow-sm after:content-['%'] after:absolute after:text-[0.75em]">{item.value}</span></div>
              ))}
            </div>    
          </div>
          ...
          <div class="relative px-5 py-2 font-medium text-white group mt-4">
            ...
            <span class="absolute bottom-0 left-0 hidden w-10 h-20 transition-all duration-100 ease-out transform -translate-x-8 translate-y-10 bg-orange-600 -rotate-12"></span>
            <span class="absolute bottom-0 right-0 hidden w-10 h-20 transition-all duration-100 ease-out transform translate-x-10 translate-y-8 bg-orange-400 -rotate-12"></span>
            <span id="spin-wheel-result" class="relative">Gire para desvendar um cupom</span>
          </div>
        </div>
      </div>
